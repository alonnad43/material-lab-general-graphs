...
def linear_fit_elastic_region(x: np.ndarray, y: np.ndarray, max_strain: float = 0.5):
    """Return slope, intercept, r, and fit range for the elastic region."""
...
def plot_bending_graphs() -> None:
    """Generate bending test graphs for all materials."""
    ...
    for i, (xcol, ycol) in enumerate(col_sets):
        ...
        if not x.size or not y.size:
            continue
        slope, intercept, r_value, x_fit, _y = linear_fit_elastic_region(x, y)
        if len(x) > 50:
            ax.plot(x, y, linestyle='-', linewidth=1.2, label=f'Specimen {i+1}')
            ax.plot([x[0], x[-1]], [y[0], y[-1]], 'o', markersize=4, alpha=0.7,
                    color=ax.get_lines()[-1].get_color())
        else:
            ax.plot(x, y, marker='o', linestyle='-', markersize=2, alpha=0.4,
                    label=f'Specimen {i+1}')
        if slope is not None and r_value ** 2 >= 0.5:
            ax.plot(x_fit, slope * x_fit + intercept, '--',
                    label=f'Specimen {i+1} elastic fit (E={slope:.1f}, $R^2$={r_value**2:.2f})')
        elif slope is not None:
            ax.plot(x_fit, slope * x_fit + intercept, '--',
                    label=f'Specimen {i+1} elastic fit (E={slope:.1f})')
    ...
    for fname, mat, method in files:
        ...
        if slope is not None and r_value ** 2 >= 0.5:
            ax.plot(x_fit, slope * x_fit + intercept, '--',
                    label=f'{label} elastic fit (E={slope:.1f}, $R^2$={r_value**2:.2f})')
        elif slope is not None:
            ax.plot(x_fit, slope * x_fit + intercept, '--',
                    label=f'{label} elastic fit (E={slope:.1f})')
    ax.set_xlabel('Extension (mm)', fontsize=label_fontsize)
    ax.set_ylabel('Load (N)', fontsize=label_fontsize)
