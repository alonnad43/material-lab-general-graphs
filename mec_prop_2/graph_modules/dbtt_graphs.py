...
def sigmoid(T, A, k, T0, E_min):
    """Sigmoid function for DBTT fitting."""
    return A / (1 + np.exp(-k * (T - T0))) + E_min


def plot_dbtt_graph() -> None:
    """Generate energy vs temperature graph with transition marker."""
    ...
    df = load_csv('Charpy_Impact_Test_Ductile_to_Brittle_Transition',
                  'Fracture_Energy_Temperature_for_Ductile_britle_transtion.csv')
